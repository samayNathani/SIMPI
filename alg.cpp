...
matrix matrixadd(matrix A, matrix B, matrix C, int par_id, int par_count)
{
    ...
    for (int i = start; i< end; i++){
        for (int j =0;j < Acol; j++ ){
            ...
            // }
    // printf("YESS\n");
            C.set(j+i*Acol,(B.get_algbera(j+i*Acol) + A.get_algbera(j+i*Acol)));
    // printf("YE\n");
        }
...
matrix matrixsubtract(matrix A, matrix B, matrix C, int par_id, int par_count)
{
    ...
    for (int i = start; i< end; i++){
        for (int j =0;j < Acol; j++ ){
            ...
            //     printf("LESSS GOOO = %d\n", j+i*Arow);
            // }
            C.set(j+i*Acol, (A.get_algbera(j+i*Acol) - B.get_algbera(j+i*Acol)));
        }
...
void get_print_matrixval(matrix A, int par_id, int par_count)
{
...
// // // A = (matrix)mmap(0, sizeof(matrix) * 16, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
// // // B = (matrix)mmap(0, sizeof(matrix) * 16, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
// // // C = (matrix)mmap(0, sizeof(matrix) * 16, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
