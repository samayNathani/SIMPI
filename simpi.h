...
class simpi {
 ...
 private:
  ...
  std::string sync_shared_mem_name;  // this shared meory holds: shared mem name
                                     // for last allocated matrix id, synch
  ...
  {
    int synchid = ready[par_count] +
                  1;  // synchid is progressing with each synch - I know, one
                      // from section 1 and 3 had a similar idea.
    ...
    while (1) {
      ...
      for (int i = 0; i < par_count; i++) {
        if (ready[i] <
            synchid)  //"less than" is super important here. Do you know why?
                      // Because one process could run ahead and
      ...
      if (breakout == 1) {
        ready[par_count] =
            synchid;  // and here we increment the additional variable
        break;
      }
    }
  }
...
class matrix  // similar stuff for vector
{
 ...
 public:
  ...
  double& get(int x, int y) { return arr[x + x * y]; }
};
